...
def length_gate(text, final, minimum_words=3):
    """English replay gate; final text always reaches fallback processing."""
    units = len(re.findall(r"\b[\w]+(?:['’-][\w]+)*\b", text))
    return {'allow': final or units >= minimum_words, 'words': units,
            'minimum_words': minimum_words,
            'reason': 'final-bypass' if final else ('length-pass' if units >= minimum_words else 'too-short')}


def expanded_cases(records, split):
    cache, old = build_cases(records, split)
    cases = []
    for db in sorted({r['db_id'] for r in records}):
        ordered = sorted([r for r in records if r['db_id'] == db],
                         key=lambda r: (-len(r['question'].split()), str(r['question_id'])))
        for r in ordered[:2]:
            words = r['question'].split()
            cases.append({'id': f"long:{db}:{r['question_id']}", 'db_id': db,
                          'kind': 'long-public-question', 'target_id': None, 'context': [],
                          'snapshots': [' '.join(words[:1]), ' '.join(words[:len(words)//3]),
                                        ' '.join(words[:2*len(words)//3]), r['question']]})
        # Two real questions in one synthetic turn; first complete before second begins.
        a, b = sorted(ordered, key=lambda r: (len(r['question'].split()), str(r['question_id'])))[:2]
        bw = b['question'].split()
        cases.append({'id': f"compound:{db}", 'db_id': db, 'kind': 'synthetic-two-questions',
                      'target_id': None, 'context': [], 'expected_questions': [a['question'], b['question']],
                      'snapshots': [a['question'].split()[0], a['question'],
                                    a['question']+' Also, '+' '.join(bw[:max(2,len(bw)//2)]),
                                    a['question']+' Also, '+b['question']]})
    for c in old:
        if c['kind'] == 'synthetic-context':
            # Hold text constant between penultimate and final to isolate final flag.
            c['snapshots'] = [c['snapshots'][0], c['snapshots'][1], c['snapshots'][2], c['snapshots'][2]]
            cases.append(c)
    assert len(cases) == 32
    return cache, cases
...
def main():
    ...
    ap.add_argument('--minimum-words', type=int, default=3)
    args = ap.parse_args()
    if args.minimum_words < 1:
        ap.error("minimum-words must be positive")
    ...
    cache, cases = expanded_cases(json.loads(records_bytes), json.loads(split_bytes))
    ...
    save('manifest.json', {'model': model_name, 'embedding_model': MODEL,
         ...
         'max_calls': 128, 'minimum_words': args.minimum_words, 'output_tokens': 256, 'retries': 0,
    ...
    with httpx.Client(trust_env=False, timeout=30) as client:
        for case in cases:
            for step in range(len(case['snapshots'])):
                started = time.perf_counter()
                row = {'case': case['id'], 'kind': case['kind'], 'snapshot': step,
                       'payload': payload(case, step), 'target_id': case['target_id']}
                row['gate'] = timed('length_gate', lambda: length_gate(
                    row['payload']['text'], row['payload']['final'], args.minimum_words),
                    case=case['id'], snapshot=step)
                if not row['gate']['allow']:
                    row.update(status='gated', elapsed_ms=round((time.perf_counter()-started)*1000, 3))
                    log('results.jsonl', row)
                    log('timing.jsonl', {'step': 'snapshot_total', 'case': case['id'],
                                        'snapshot': step, 'elapsed_ms': row['elapsed_ms']})
                    results.append(row)
                    save('progress.json', {'completed': len(results), 'planned': 128})
                    continue
    ...
    save('summary.json', {'snapshots': len(results), 'errors': sum(r['status'] == 'error' for r in results),
         'gated': sum(r['status'] == 'gated' for r in results),
         'llm_calls': sum('usage' in r or r['status'] == 'error' for r in results),
