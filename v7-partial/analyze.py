...
def analyze(root):
    ...
    for stage in ('model_load', 'index_embedding', 'query_embedding', 'cosine_search', 'builder_llm', 'snapshot_total'):
    ...
    out['boundary'] = 'Exact replay retrieval only, not semantic hit accuracy; no latency savings measured.'
    ...
    for r in rows:
        lines.extend([f"## {r['case']} / snapshot {r['snapshot']}",
                      ...
                      'Builder: '+json.dumps(r.get('builder', {'error': r.get('error_type')}), ensure_ascii=False),
                      'Raw top candidate: '+json.dumps(r['raw'][0], ensure_ascii=False),
